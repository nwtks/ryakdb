...
module TempTable =
    ...
    let newTempTable fileMgr tx schema =
        let ti =
            TableInfo.newTableInfo (nextTableName ()) schema

        tx.BufferMgr.PinNew ti.FileName FileHeaderFormatter.format
        |> tx.BufferMgr.Unpin

        { OpenScan = fun () -> openScan fileMgr tx ti }
...
module Materialize =
    let newTempRecordPage tx schema tblcount =
        let ti =
    ...
    let rec loopTempRecordPage fileMgr tx schema src temps scan tblcount sortFields trp =
        ...
        if flag <> -1 then
            ...
            newTempRecordPage tx schema nextTblcount
