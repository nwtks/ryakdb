...
module FileHeaderPage =
    ...
    let getVal offset sqlType state =
...
module RecordPage =
    ...
    let newRecordPage tx blockId tableInfo doLog =
        let offsetMap, pos =
...
module RecordFormatter =
    ...
    let makeDefaultRecord tableInfo (offsetMap: Map<string, int32>) buffer pos =
        tableInfo.Schema.Fields()
        |> List.iter (fun field ->
            buffer.SetValue
...
module RecordFile =
    ...
    let formatFileHeader fileMgr tx fileName =
    ...
    let newRecordFile fileMgr tx doLog (tableInfo: TableInfo) =
