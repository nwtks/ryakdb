...
module HashIndex =
    ...
    let beforeFirst fileMgr tx indexInfo keyType searchRange =
        ...
        let ti =
            ...
            |> TableInfo.newTableInfo fileMgr tblName

        let rf = ti.OpenFile tx false
        if rf.FileSize() = 0L
    ...
    let getDataRecordId indexInfo state =
        ...
        | Some ({ RecordFile = rf }) ->
            ...
            RecordId.newBlockRecordId rid (indexInfo.TableName + ".tbl") blockNo
        | _ -> failwith "You must call beforeFirst()"
...
module Index =
    let newIndex fileMgr tx indexInfo keyType =
